...
% musical notes in each of the STR which vaguely correspond to Helmholtz
% pitch notation (http://en.wikipedia.org/wiki/Helmholtz_pitch_notation).
%
% S = SING('STR1 ...',BPM) alters the temp of the tune (in beats per
% minute). The default value is 60bpm.
%
% For example, S = SING('A'), produces a function corresponding to the note
% A, S = SING('B') produces a B, and so on.
%
% Basic chords are provided, for example S = SING('CEG') will produce a 
% major triad (http://en.wikipedia.org/wiki/Major_chord). The convention
% is for the kth note to have 1/k times the amplitude of the first.
% 
...
%
% The SING keyboard has three octaves (in A to G). S = SING('C-') produces
% a low C, S = SING('C') produces a middle C, and S = SING('C+') produces a
% high C.
%
...
s = chebfun('x',[0 t0/4]); % quarter note
...
while ~isempty(str)
    ...
    while ~isempty(strk)
        ...
        if strcmp(strjk,'R'), songtmp = [songtmp ; 0*q]; break, end
