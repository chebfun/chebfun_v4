...
%% 
% (Chebfun2 example Optimization/GlobalMinimum.m)

%% The SIAM 100-Dollar, 100-Digit Challenge
% In February 2002, an article in SIAM News by Nick Trefethen set a 
% challenge to solve ten problems each to ten digits of precision 
% (the solution of each problem was a real number) [1].  One of the
% problems was to find the global minimum of the complicated function

f = @(x,y) exp(sin(50*x)) + sin(60*exp(y)) + sin(70*sin(x)) +... 
    sin(sin(80*y)) - sin(10*(x+y)) + (x.^2+y.^2)./4; 

x = linspace(-1,1); [xx yy]=meshgrid(x);
surf(xx,yy,f(xx,yy)), title('The complicated function','FontSize',16) 

%% 
% Since the term $(x^2+y^2)/4$ grows away from $(0,0)$ while the other 
% terms remain bounded, it can be shown that the global minimum occurs in 
% $[-1,1]^2$ [2].

%%
% The function is complicated and oscillatory, but of rank $4$, as can be 
% seen by rearranging its terms and using the identity 
...
%% 
% For details about what we mean by the rank of a function see
% [3].   The minimum was found in [2] to 10,000 digits, and here 
% are the first 16:
...
%% References
% [1] Lloyd N. Trefethen, A 100-Dollar, 100-Digit Challenge, SIAM News,
% 35 (2002). 
%%
% [2] Folkmar Bornemann, Dirk Laurie, Stan Wagon and Joerg Waldvogel, 
