...
%%
% (Chebfun2 example VectorCalculus/AutonomousSystems.m)

%% Autonomous systems
% An autonomous system is a system of ordinary differential equations which
% does not explicitly depend on the independent variable. Such a system can be written in the
% form of $dy/dt(t) = f(y(t))$ where $y(t)$ takes values in $R^n$.  
% In this example we restrict ourselves to
% $n = 2$ so that we can plot the phase plane (vector field) and trajectory
% of $y(t)$ given initial conditions using Chebfun2.
...
%% 
% The chebfun2v F is a vector field, and the overloaded ode45 command 
% is used to compute its trajectories. Each trajectory is represented by a 
% complex valued chebfun y parameterised by the real time variable t. 
...
% that models a damped oscillator. It has three critical points, two stable
% and one unstable.  Here is the phase portrait with a carefully selected
% trajectory:
