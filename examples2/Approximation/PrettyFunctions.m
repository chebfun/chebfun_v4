...
%%
% (Chebfun2 example Approximation/PrettyFunctions.m)

%% Pretty functions
% A pretty function of two variables is one that can be used to easily 
% display an aesthetically appealing image.  Chebfun2 has half a dozen
% plotting commands such as plot, plot3, surf, surfc, mesh, and waterfall. 
% In this example we use some of them to make pretty pictures.

%% Contour and pivot plot 
% The contour plot of a chebfun2 displays level curves and is analogous
% to the standard Matlab contour command.  On the other hand, the pivot plot, with 
% syntax plot(f,'.'), of a chebfun2 displays the locations of the pivots used during the
% construction process.  The number of pivots is equal to the rank of
% the chebfun2 object. Here are a selection of our favourite Chebfun2 contour 
% plots along with the pivot locations that were selected during
% construction. 
...
% height of the surface above (x,y) is equal to the value of f(x,y). Here
% are four examples.
...
%% Waterfall 
% The waterfall plot displays the lines on which the function was sampled
% in order to form the chebfun2 approximant. The positions of the lines were
% calculated by the adaptive construction process. Here is Franke's functions and
% the lines on which it was sampled to form an approximation. 
...
%% Phase portraits
% For plots of complex valued functions see the Phase portrait example 
% [Townsend 2013].

%% References
% [Townsend 2013] Chebfun2 example: ComplexAnalysis/PhasePortraits, March
% 2013.