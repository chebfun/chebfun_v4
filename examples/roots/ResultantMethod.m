...
% Here we explore a new algorithm in Chebfun2 by Nakatsukasa, 
% Noferini, and the Example's author for computing the solutions to 
% bivariate rootfinding problems: 
%
% $f(x,y) = g(x,y) = 0, \quad (x,y)\in [-1,1]\times[-1,1],$
%
% where $f$ and $g$ are represented by chebfun2 objects. Numerically, 
% this problem can be solved by many different methods such as the homotopy
% continuation method [3], the two-parameter eigenvalue method [1], the 
% resultant method [2], and Marching Squares [4,5]. More information 
% on existing numerical methods for bivariate rootfinding can be found 
% in [2]. 
...
% chebfun2/roots, but for details of the new algorithm we refer the reader to
% [2]. 
...
%%
% By default the new Chebfun2 roots command employs the resultant method if
% the degrees of $f$ and $g$ are less than 200 and Marching Squares, 
% otherwise. However, we always treat the computed zeros from
% Marching Squares with suspicion. 
...
%% 
% The new algorithm based on the resultant method does not compute the 
% zero contours of $f$ and $g$, but we display them throughout this 
% Example to visualise the computed solutions. 

%% Example 2: Coordinate alignment
% Traditionally, resultant methods have numerical difficulties when 
% many solutions are aligned along a coordinate direction, and some implementations 
% use a change of variables to prevent any solutions aligning in this way. 
% In [2] a careful case-by-case study is provided to show that these numerical
...
%% 
% The diagram above shows that we have found all the solutions, and we can
% get a rough idea of the error in the computed solutions by computing
% the residue:
...
%% References: 
%% 
% [1] P. A. Browne, Numerical methods for two parameter eigenvalue 
% problems, PhD. Thesis, University of Bath, 2008.
%% 
...
%%
% [3] A. J. Sommese and C. W. Wampler, The numerical solution of systems 
% of polynomials, World Scientific, 2005.
%% 
