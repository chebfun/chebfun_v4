...
% follows.  We begin by defining a domain, the linear function x and a
% nonlinear chebop N:
[d,x,N] = domain(-1,1);

%%
% We assign the differential equation and boundary conditions fields: 
N.op = @(u) 0.01*diff(u,2) + 2*(1-x.^2).*u + u.^2;
N.bc = 'dirichlet';

%%
% Finally we set an initial guess:
N.guess = 2*(x.^2-1);
...
%%
% Running from a different initial guess gives a different
% solution:
