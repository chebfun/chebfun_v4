...
%% 
% Two shapes are equivalent if one can be obtained from the other
% by translating, scaling and rotating. Before comparison we thus:
%%
...
% depends on this correspondence.  A different 
% correspondence gives a different error.   In the continuous case this 
% correspondence becomes the parameterisation. A different parameterisation
% of the two curves gives a different error. This continuous version of 
% Procrustes (as implemented in this example) is therefore
% more of an 'eye-ball' check rather than a robust statistical analysis.
