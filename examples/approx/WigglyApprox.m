...
%%
% Ken Lord, whose doctoral supervisor was the Chebyshev
% technology expert Charles Clenshaw, has explored functions
% of the form
%
% f(x) = T_m(x) + T_m+1(x) + ... + T_n(x),
%
% where T_k is the kth Chebyshev polynomial, as challenging
% functions for minimax approximation by polynomials of
% lower order.  We can construct such functions in a single
% Chebfun command:
...
grid on, title('f(200,220)',FS,16)
subplot(2,2,2), plot(f{.995,1},LW,1.6)
grid on, title('closeup',FS,16), xlim([.995 1])
p = remez(f,m-1); err = f-p;
subplot(2,2,3), plot(err,'r',LW,.8)
grid on, title('f - p',FS,16)
subplot(2,2,4), plot(err{.995,1},'r',LW,1.6)
grid on, title('closeup',FS,16), xlim([.995 1]), toc
