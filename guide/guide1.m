...
%%
% The mathematical foundations of the Chebfun are for the most part well
...
% through 49 Chebyshev points, i.e., a polynomial of degree 48.  These
...
% quadrature -- see Section 2.1), and it is interesting to compare it to
% the exact answer from calculus:
...
%%
% The function looks complicated, but it is actually a polynomial of
% surprisingly small degree:
  length(g)
%%
...
% computed zeros are accurate to about 13 digits:
  besselj(0,r)
...
% "fun", and funs are implemented as a subclass of chebfuns.  This
% enhancement of Chebfun was developed initially by Ricardo Pachon during
% 2006-2007, then also by Rodrigo Platte starting in 2007 [Pachon, Platte
% and Trefethen 2009]. Essentially funs are the "classic chebfuns" for
% smooth functions on [-1,1] originally implemented by Zachary Battles.
...
% from another angle with "struct", Matlab's command for seeing the various
% fields within an object:
  struct(f)
%%
% This output again shows that f consists of two funs with breakpoints at
% -1, 1, and a number very close to 0.3.  The "imps" field refers to
% "impulses", which relate to values at breakpoints, including possible
% information related to delta functions, discussed in Section 2.4.  The
% "trans" field is 0 for a column chebfun and 1 for a row chebfun (Section
% 1.6 and Chapter 6).  The "jacobian" and "ID" fields are used for
% Automatic Differentiation (Chapter 10).
...
%%
% As always, h may look complicated to a human, but to Chebfun, it is just
% a function.  Here are its mean, standard deviation, minimum, and maximum:
...
%%
% In a chebfun computation, splitting can be turned on and off freely to
% handle different functions appropriately.  The default or "factory" value
% is splitting off; see Chapter 8.
...
% diverge to infinity: the credit for these innovations belongs to Nick
% Hale, Rodrigo Platte, and Mark Richardson. For example, here is a
% function on the whole real axis,
...
%%
% Here is an example of a function that diverges to infinity, which we can
% capture by including the flag 'blowup 2' (try help blowup for details):
...
% and matrices.  The same is true of chebfuns. The chebfuns shown so far
% have all been in column orientation, which is the default for Chebfun,
% but one can also take the transpose, compute inner products, and so on:
...
%%
  x'*x
%%
% One can also make matrices whose columns are chebfuns or whose rows are
% chebfuns, like this:
...
% This guide is produced in Matlab using the "publish" command. The
% formatting is rather simple, not relying on TeX features or other fine
% points of typesetting.  To publish a chapter for yourself, make sure the
% Chebfun guide directory is in your path and then type, for example,
% "open(publish(guide1))". Before publishing, we recommend executing
% "guidedefaults".
