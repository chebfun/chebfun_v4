...
%% 3.1 roots
% Chebfuns come with a global rootfinding capability -- the ability to find
...
% at WolframMathWorld.  (The reason for the "real" command is a bug in
% Matlab: Matlab's "airy" command gives spurious nonzero imaginary parts,
% at the level of rounding errors, when applied to some real arguments,
% such as x=-2.)
...
%%
% We saw this effect already in Section 1.4. Another similar effect shown
% in that section occurs with min(f,g) or max(f,g).  Here, breakpoints are
% introduced at points where f-g is zero:
...
% Local extrema of smooth functions can be located by finding zeros of the
% derivative.  For example, here is a variant of the Airy function again,
% with all its extrema in its range of definition located and plotted.
...
%%
% This method will find non-smooth extrema as well as smooth ones, since
% these correspond to "zeros" of the derivative where the derivative jumps
% from one sign to the other.  Here is an example.
  x = chebfun('x');
  f = exp(x).*sin(30*x);
  g = 2-6*x.^2;
  h = max(f,g);
...
%%
% The system computes such a result by checking the values of f at all
...
% norm(f,1) is the integral of the absolute value, and the system computes
...
%%
% The simplest example is a chebfun that is truly intended to correspond to
% a polynomial.  For example, the chebfun
f = 1+16*x.^2;
...
%%
% The situation for more general chebfuns is more complicated.  For
% example, the chebfun
g = exp(x).*f(x);
...
% command roots(g,'complex') first effectively does roots(g,'all'), then
% returns only those roots lying inside a particular chebfun ellipse -- we
% take the one corresponding to delta equal to the square root of the
% Chebfun tolerance parameter eps.
...
roots(f,'complex')
%%
% Notice that the first three imaginary roots are located with about 10, 8,
% and 6 digits of accuracy, while the fourth does not appear in the list at
% all.
...
%
% [Boyd 2002] J. A. Boyd, "Computing zeros on a real interval through
% Chebyshev expansion and polynomial rootfinding", SIAM Journal on
% Numerical Analysis 40 (2002), 1666-1682.
%
% [Good 1961] I. J. Good, "The colleague matrix, a Chebyshev analogue of
% the companion matrix", Quarterly Journal of Mathematics 12 (1961), 61-68.
%
% [Specht 1960] W. Specht, "Die Lage der Nullstellen eines Polynoms. IV",
% Mathematische Nachrichten 21 (1960), 201-222.
