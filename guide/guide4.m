...
% (The same results could be generated by replacing
% chebpolyval([1 zeros(1,n)]) by (-1).^(n:-1:0).)

  subplot(2,2,1), plot(chebfun((-1).^(2:-1:0)))
  subplot(2,2,2), plot(chebfun((-1).^(3:-1:0)))
  subplot(2,2,3), plot(chebfun((-1).^(15:-1:0)))
  subplot(2,2,4), plot(chebfun((-1).^(50:-1:0)))
...
%%
% We have just seen that the command chebfun((-1).^(N:-1:0)) returns a chebfun
