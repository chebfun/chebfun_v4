...
% The chebfun system is founded on the mathematical subject of
% approximation theory, and in particular, on Chebyshev series and
% interpolants. Conversely, it provides a simple environment in which to
% demonstrate these approximants and other approximation ideas.
...
% entirely to Chebyshev polynomials: [Rivlin 1974] and [Mason & Handscomb
% 2003]. A Chebfun-based book on approximation theory and its computational
% applications is in preparation [Trefethen 2010].
...
% of spectral methods that Chebyshev techniques are best known today [Boyd
% 2001], [Trefethen 2000], [Canuto et al. 2006/7].
...
%%
% By *Chebyshev points* we shall mean as usual the set of points in [-1,1]
% defined by
%
%         x(j) = -cos(j pi/N),    0 <= j <= N,
%
% where N is an integer >= 1.  (If N=0, we take x(0)=0.) Through any data
% values f(j) at these points there is a unique polynomial interpolant p(x)
% of degree <= N, which we call the *Chebyshev interpolant*. In particular,
% if the data are f(j) = (-1)^(n-j), then p(x) is T_N, the Nth Chebyshev
% polynomial, which can also be defined by the formula T_N(x) = cos(N
% acos(x)).  In Chebfun, the command "chebpoly(N)" returns a chebfun
% corresponding to T_N, and "poly" returns coefficients in the monomial
% basis 1,x,x^2,.... Thus we can print the coefficients of the first few
...
%
% except that for k=0, the constant changes from 2/pi to 1/pi. One way to
% approximate a function is to form the polynomials obtained by truncating
...
% exact Chebyshev coefficients.   Instead chebfuns are based on Chebyshev
...
% points rather than its Chebyshev coefficients, but this hardly matters to
% the user, and both representations are exploited for various purposes
...
splitting off
%%
% since our purpose is to explore global Chebyshev interpolants without any
...
%%
% Like "poly", "chebpoly" returns a row vector with the high-order
% coefficients first. Thus this computation reveals the identity x^3 =
% (1/4)*T_3(x) + (3/4)*T_1(x).
...
%%
% Of course, machine precision is defined relative to the scale of the
% function:
...
% in 10 or 20 points, taking N to be even to avoid including a value 0 at
...
% does not go away as N -> infty. We can zoom in on the overshoot region by
% resetting the axes:
...
%%
% The second plot is jagged, not because there is anything wrong with the
% underlying chebfun but because we have zoomed in very closely on the
% result of a "plot" command.  One way to get it right is by using
% Chebfun's { } feature, which enables one to construct a chebfun
% corresponding to a subinterval. (Another way would be to use the
% 'interval' flag in the "plot" command.)
...
%%
% What is the amplitude of the Gibbs overshoot for Chebyshev interpolation
% of a step function?  We can find out by using "max":
...
%%
% The overshoot converges to a number 1.282283455775.... [Helmberg & Wagner
% 1997].

%% 4.4 Smoothness and rate of convergence
% The most basic principle in approximation theory is this: the smoother
% the function, the faster the convergence as N -> infty. What this means
...
% [Battles & Trefethen 2004], and all will be discussed in [Trefethen
% 2010].  Let f be a continuous function on [-1,1], and let p denote its
% interpolant in N Chebyshev points and p* its best degree N approximation
...
%%
% *THEOREM 2*.  If Let f, f', ..., f^(k-1) be absolutely continuous for
% some k >=1, and let f^(k) be a function of bounded variation.  Then *||*
...
% then *||* f - p *||* = O(r^(-N)) as N -> infty.

%%
% More precisely, if abs(f(z)) <= M in the ellipse, then the bound on the
% right can be taken as 4Mr^(-n)/(r-1).

%%
% The next theorem asserts that Chebyshev interpolants can be computed by
% the barycentric formula [Salzer 1972].  The notation SUM" denotes the sum
...
%%
% See [Berrut & Trefethen 2005] for information about barycentric
% interpolation.

%%
% The final theorem asserts that the barycentric formula has no difficulty
% with rounding errors.  Our "theorem" is really just a placeholder; see
...
% idea, and it is certainly interesting to be able to compute them. Chebfun
% makes this possible with the command "remez", named after Evgeny Remez,
% who devised the standard algorithm for computing these approximations in
% 1934. This capability is due to Ricardo Pachon; see [Pachon & Trefethen
% 2009].
...
%%
% Notice that although the best approximation has a smaller maximum error,
% it is a worse approximation for almost all x.

%%
% The Chebfun code "remez" does not (yet) compute rational approximants. If
% your function is smooth, however, you can compute best approximants by
% Caratheodory-Fejer approximation using the code "cf" due to Joris Van
% Deun.  For example:
f = chebfun('exp(x)');
[p,q] = cf(f,5,5);
r = p./q;
...
% practical work, but like best approximations, they are certainly
% interesting.

%%
% In Chebfun, we can compute them with the overloaded "interp1" command.
...
% Chebfun contains three different programs, at present, for computing
...
% and Graves-Morris 1996].  (This is the so-called "Clenshaw-Lord"
...
%%
% Mathematically, f has poles in the complex plane at +-i, +-3i, +5i, and
...
% different set of points). This capability was developed by Ricardo Pachon
% and Pedro Gonnet [Pachon & Gonnet 2010].  The results are similar:
...
%%
% The third option for rational approximation is Caratheodory-Fejer
% approximation, realized in the code "cf" by Joris Van Deun [Trefethen &
% Gutknecht 1983]. As mentioned in Section 4.6, CF approximants often agree
% with best approximations to machine precision if f is smooth. We explore
% the same function yet again, and this time obtain an equioscillating
% error curve:
[p,q] = cf(f,40,4);
r = p./q;
norm(f-r)
...
%
% [Fox & Parker 1966] L. Fox and I. B. Parker, Chebyshev Polynomials in
% Numerical Analysis, Oxford U. Press, 1968.
%
...
%
% [Pachon & Gonnet 2010] R. Pachon and P. Gonnet, manuscript in
% preparation.
%
...
%
% [Platte, Trefethen & Kuijlaars 2009] R. P. Platte, L. N. Trefethen and A.
% B. J. Kuijlaars, "Impossibility of approximating analytic functions from
% equispaced samples", manuscript, 2009.
%
...
%
% [Trefethen 2000] L. N. Trefethen, Spectral Methods in Matlab,  SIAM,
% 2000.
%
