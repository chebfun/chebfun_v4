...
%% 8.1  Introduction
% Like any software system, Chebfun is based on certain
...
% like the maximum number of points at which a function will
% be sampled before
% Chebfun gives up trying to resolve it.  A starting point in
...
% review will also serve to deepen the user's understanding of Chebfun
% by poking about a bit at its edges.
...
% is simply to represent complicated functions, and perhaps to
% compute their integrals, especially when they are
...
% it is very convenient that Chebfun can be used so easily
...
% especially high degrees.  On the machines of 2011, Chebfun is perfectly
...
% at the heart of the chebop system.  When Chebfun solves a differential
...
%%
% There are certainly cases where weakening the tolerance
...
%%
% 'sampletest' controls whether a function is evaluated at an extra
% point as a safety check of convergence.  With the default 
% 'on' value, this test is indeed carried out.

%%
% 'blowup' relates to the construction of chebfuns
% that diverge to infinity, as described in Chapter 9.
% blowup=0 is used for no singularities, blowup=1 if for functions
% with poles (blowups with a negative integer power) and blowup=2 for
% functions with branch points (blowups with an arbitrary power).

%%
% 'chebkind' is set by default to 2, but can be changed to 1 to
% force Chebfun to use Chebyshev points of the first kind (i.e., roots
% of Chebyshev polynomials) rather than the second kind (extrema).
% This feature is experimental and may not do what you expect.

%%
% 'extrapolate" can be turned on to avoid evaluating functions
% at endpoints if this may lead to trouble with NaN or inf.
% For example, here we get a warning message
% because the value is 0 at the x=0 and 1 elsewhere,
...
%%
% 'addepth' stands for 'automatic differentiation depth', and limits
% how deep an automatic differentiation stack can go in order to
% limit memory use.
