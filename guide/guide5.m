...
%%
% Such plots make pretty pictures, but as always with chebfuns, the
...
% An easier method is to use the "vertcat" feature of Chebfun, in which a
% construction like [f; g; h] constructs a single chebfun with the same
% values as f, g, and h, but on a domain concatenated together.  Thus if
% the domains of f, g, h are [a,b], [c,d], and [e,d], then [f; g; h] has
...
% If s is a real parameter and z(s) is a complex function of s, then we can
% define a contour integral in the complex plane like this:
%
%        INT f(z(s)) z'(s) ds
%
...
% points z1 and z2 is path-independent.  To verify this, we can compute the
% same integral over the straight segment going directly from 0 to -1+0.5i:
...
% *Cauchy integral formula*, (1/2i*pi) times the integral of a meromorphic
...
% its Laurent expansion at z0.  For example, the function exp(z)/z^3 has
% Laurent series z^(-3) + z^(-2) + (1/2)z^(-1) + (1/6)z^0 + ... at the
...
%%
% Notice that we have just computed the degree 2 Taylor coefficient of
% exp(z).

%%
% When Chebfun integrates around a circular contour like this, it does not
% take advantange of the fact that the integrand is periodic.  That would
% be Fourier analysis as opposed to Chebyshev analysis, and a "fourfun"
% system would be more efficient for such problems [Davis 1959].  Chebyshev
...
%%
% The integral of f(z) = log(z)tanh(z) around this contour will be equal to
% 2i*pi times the sum of the residues at the poles of f at +-0.5i*pi.
...
%
% Here are some further examples of computations with Cauchy integrals.
% The Bernoulli number B_k is k! times the kth Taylor coefficient of
% z/((exp(z)-1). Here is B_10 compared with its exact value 5/66.
...
% argument of f as the boundary is traversed.  In principle it should be
% possible to calculate this by the Matlab commands "angle" and "unwrap",
% but these have not yet been overloaded for chebfuns.) For example, the
% function f(z) = sin(z)^3 + cos(z)^3 clearly has no poles; how many zeros
% does it have in the disk about 0 of radius 2? The following calculation
% shows that the answer is 3:
...
%%
% You can find a code "greetingcard" for producing images like this in the
...
%
% [Delves & Lyness 1967] L. M. Delves and J. N. Lyness, "A numerical method
% for locating the zeros of an analytic function", Mathematics of
% Computation 21 (1967), 543-560.
%
