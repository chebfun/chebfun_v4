...
%%
% (Chebfun has no trouble if xmax is increased to 10^5 or 10^10.)  Here is
% a plot comparing Li(x) with pi(x):
  clf, plot(Li,'m')
  p = primes(xmax);
...
%%
% Here's an example of how differentiation can be used in a numerical
% computation.  In Chapter 1 we computed some zeros of a Bessel function,
% which came out to about 13 digits of accuracy:
  g = chebfun(@(t) besselj(0,t),[0,100]);
  r = roots(g); r = r(1:5)
  Jr = besselj(0,r)

%%
% We can get a couple more digits of accuracy by taking one step of
% Newton's method:
  gprime = diff(g);
...
%%
% There is no need to stop at 4 points, however. Here we use 1000 Gauss
% quadrature points:
...
% Welsch 1969], and this is what Chebfun does when the number of points is
% less than 128.  For large grids, however, Chebfun uses the different
% algorithm of Glaser, Liu and Rokhlin [Glaser, Liu & Rokhlin 2007] with
% enhancements due to Hale and Sheehan Olver (unpublished).
...
% singularities at one or both endpoints, and are used internally in
% Chebfun for integration of chebfuns with singularities (Chapter 9).

%%
% As explained in the help texts, all of these operators work on general
% intervals [a,b], not just on [-1,1].
...
% algorithm for the calculation fo the roots of special functions", SIAM
