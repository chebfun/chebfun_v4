...
for k = 1:numel(Fout)
    ...
    if numel(Fout) == numel(F) 
        Fout(k).jacobian = anon('diag1 = diag(-besselj(nu+1,F)+nu*Fout./F); der2 = diff(F,u); der = diag1*der2; nonConst = ~der2.iszero;', ...
            {'nu' 'F' 'Fout'},{nu(k) F(k) Fout(k)},1);
    else
        Fout(k).jacobian = anon('diag1 = diag(-besselj(nu+1,F)+nu*Fout./F); der2 = diff(F,u); der = diag1*der2; nonConst = ~der2.iszero;', ...
            {'nu' 'F' 'Fout'},{nu(k) F Fout(k)},1);
    end
    Fout(k).ID = newIDnum();
end
