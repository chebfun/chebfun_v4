...
function [p,q,r,mu,nu,poles,residues] = ratinterp( f , m , n , NN , xi_type , tol )
...
%
%   See also CHEBFUN/INTERP1, DOMAIN/INTERP1.
...
%   See http://www.maths.ox.ac.uk/chebfun/ for Chebfun information.
    ...
    if nargin < 4 || isempty(NN)
        if nargin > 5 && ~isempty(xi_type) && ~isstr(xi_type)
            N = length( xi_type ) - 1;
        elseif isfloat( f )
            N = length( f ) - 1;
        else
