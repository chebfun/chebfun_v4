...
%
% P = CF(F,M): degree M polynomial CF approximant to chebfun F defined 
%              in [a b] (which must consist of just a single fun)
%
...
%
% Note: at present this is restricted polynomial CF approximation only.
%
...
a = chebpoly(f);
if m==M-1
   p = chebfun(chebpolyval(a(2:M+1)),d); 
   return
end
c = a(M-m:-1:1);
[V,D] = eig(hankel(c));
