...
[ p , q , r , mu , nu ] = ratinterp2( cf , 10 , 10 , [] , 'type1' );
pass(5) = (mu == 4) & (nu == 2) & (max(abs(sort(roots(q,'all'))-[-0.2;2.2])) < 1e-10);

[ p , q , r , mu , nu ] = ratinterp2( cf , 10 , 10 , [] , 'type2' );
pass(6) = (mu == 4) & (nu == 2) & (max(abs(sort(roots(q,'all'))-[-0.2;2.2])) < 1e-10);

[ p , q , r , mu , nu ] = ratinterp2( cf , 10 , 10 , [] , 'equi' );
...
[ p , q , r , mu , nu , poles ] = ratinterp2( d , f(1+exp(2i*pi*(0:N-1)'/N)) , 10 , 10 , N , 'type0' );
pass(8) = (mu == 4) & (nu == 2) & (max(abs(sort(poles)-[-0.2;2.2])) < 1e-10);

[ p , q , r , mu , nu ] = ratinterp2( d , f(chebpts(N,d,1)) , 10 , 10 , N , 'type1' );
pass(9) = (mu == 4) & (nu == 2) & (max(abs(sort(roots(q,'all'))-[-0.2;2.2])) < 1e-10);

[ p , q , r , mu , nu ] = ratinterp2( d , f(chebpts(N,d,2)) , 10 , 10 , N , 'type2' );
pass(10) = (mu == 4) & (nu == 2) & (max(abs(sort(roots(q,'all'))-[-0.2;2.2])) < 1e-10);

[ p , q , r , mu , nu ] = ratinterp2( d , f(linspace(0,2,N)) , 10 , 10 , N , 'equi' );
