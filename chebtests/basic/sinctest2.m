...
dom = [-10 10];
fexact = chebfun('sin(x)/x',dom, 'vectorizing');
...
fexact = chebfun('sin(x)^2/(x^2)', dom, 'vectorizing');
f = sinc(x).^2;
...
for i = 1:T
    a = 1e2*rand(2,1);
    fexact = chebfun(@(x) sin(a(1)*(x-a(2)))/(a(1)*(x-a(2))), dom, 'vectorizing');
    
    f = sinc(a(1)*(x-a(2)));
    pass(3+i) = norm(f - fexact, inf) < tol;
end
