...
x = [0 .4 1];
exact = [0 .88384 1];
pass1 = (norm(T5(x)-exact)<1e-14);

T5b = chebpoly(5,[1,9]);
xb = 4*x + 5;
pass2 = (norm(T5b(xb)-exact)<1e-14);

T100 = chebpoly(100);
pass3 = (abs(sum(T100)-(-2/9999)) < 1e-15);
...
pass4 = (norm(a(1:3)-exacta)<1e-14);

% Combined tests:

f = chebpoly(5) + .25*chebpoly(4);
exactf = [1 .25 0 0 0 0]';
pass5 = norm(exactf - chebpoly(f)) < 1e-15;
